...
small package in this repository. Its inference base depends only on NumPy and
pandas; the `training` extra adds SciPy for fitting the shared grey-box
estimators.
...
stable grey-box API additions are distributed as `elettra-core` `2.2.1`.
The shared package is pinned by the release tag `elettra-core-v2.2.1`;
only after that tag is remotely available, `elettra` pins it immutably:

```text
elettra-core[training] @ git+https://github.com/supsi-dacd-isaac/elettra-backend.git@elettra-core-v2.2.1
```
